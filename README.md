...
| 部分 | 工具 | 用途 |
|---|---|---|
| 資料擷取 | Scrapy | 讀取分頁、控制併發，將原始回應以 append-only JSONL 保存 |
...
每個階段都會先檢查既有產物，已完成的部分會跳過。若執行中斷，可使用相同指令繼續。

在預設設定下，爬取併發數為 4、請求間隔為 1 秒。一次爬取約需 45–55 分鐘；RTX 4060 上的向量化約需 15–25 分鐘。時間會隨網路、資料量與硬體狀況改變。
...
| `CRAWL_CONCURRENCY` | `4` | 同時發出的爬取請求數 |
| `CRAWL_DELAY` | `1.0` | 請求間隔秒數 |
...
本專案用於個人研究與求職作品展示，不提供商業用途。爬取預設採低併發與固定延遲；使用前請自行確認資料來源的服務條款、robots 規範與相關限制。
